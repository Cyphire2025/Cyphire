...
function FieldSet({ label, required, error, children }) {
  return (
    <div className="mb-3">
      <div className={labelCls}>{label} {required && <span className="text-cyan-300">*</span>}</div>
      {children}
      {error && <FieldError>{error}</FieldError>}
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center mb-4">
      <button type="button" onClick={() => ref.current?.click()}
        className={`relative h-28 w-28 overflow-hidden rounded-full bg-white/10 ring-2 ring-cyan-400/20 hover:ring-cyan-400/40 transition`}
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 rounded-full shadow-[0_0_40px_rgba(34,211,238,0.25)]" />
      </button>
...
const ProofUploader = React.forwardRef(function ProofUploader({ files, onAdd, onRemove }, ref) {
  ...
  return (
    <div ref={ref} tabIndex={0} className="mt-1 transition border-2 border-dashed border-white/15 rounded-xl bg-white/5 p-3 focus:ring-cyan-400 focus:outline-none">
...
function Toast({ kind = "success", title, message, onClose }) {
  return (
    <motion.div initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 8, opacity: 0 }} transition={{ duration: 0.25 }}
      className={`pointer-events-auto w-full max-w-sm rounded-xl p-3 shadow-xl ring-1 ${kind === "success" ? "bg-cyan-500/15 ring-cyan-500/30" : "bg-rose-500/15 ring-rose-500/30"}`}
