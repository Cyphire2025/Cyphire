      ...
      <div className="grid items-center gap-10 lg:grid-cols-2">
        <div>
          ...
          <motion.div initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }} className="mt-8 flex flex-wrap items-center gap-4">
            <NeonButton onClick={() => navigate("/List-Sponsorship")} title="Post a new task">List as Sponsor <Zap aria-hidden="true" className="h-4 w-4" /></NeonButton>
            <button onClick={() => navigate("/sponsorships")} title="Explore all available tasks" className="group inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 py-3 text-white/80 backdrop-blur-xl transition-all hover:bg-white/10 active:scale-95">
              Explore Sponorship Marketplace <ChevronRight aria-hidden="true" className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </button>
          </motion.div>
        </div>
    ...
    <div className="min-h-screen overflow-x-hidden bg-gradient-to-b from-[#0a0a0f] via-[#0c0c14] to-[#000] text-gray-100 antialiased">
      ...
      <main role="main">
        ...
        <section aria-labelledby="live-briefs-title" className="mx-auto max-w-screen-2xl px-6 py-16">
          <SectionHeader id="live-briefs-title" eyebrow="Live briefs" title="Fresh missions picking up signal" subtitle="A snapshot of what teams are shipping on Cyphire right now." />
